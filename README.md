...
import data from './Hw_2.json' assert{type: JSON}
...
//const minWords = Math.min(...recordData.flatMap(record => record.round.map(rnd => rnd.feedback.trim().length)));
// 找出最大字數
//const maxWords = Math.max(...recordData.map(r => r.feedback.length));
//
// 遍歷每條記錄，為每個交互創建節點和邊
recordData.forEach((record) => {
